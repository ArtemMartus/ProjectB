...
SCENARIO("Point coordinates can be accessed via getters and setters",
    "[Point]")
{
    GIVEN("A Point with 0 0 coordinates")
    {
        Point point(0, 0);
    ...
    GIVEN("Point with coordinates 4 4")
    {
        ...
        WHEN("We call its asString() method")
        {
            ...
            THEN("We get following string '(4,4)'") { REQUIRE(str == "(4, 4)"); }
        }

        WHEN(
            "We create another point with 4 4 coordinates and call isEquals() "
            "method")
        {
            Point point2(4, 4);
            REQUIRE(point2.inBounds());

            THEN("We got correct point comparison") { REQUIRE(point == point2); }
        }
        AND_WHEN(
            "We create another point with 3 4 coordinates and call isEquals() "
            "method")
        {
            Point point2(3, 4);
            THEN("We got correct point comparison") { REQUIRE(point != point2); }
    ...
    GIVEN("A point with -1 -1 coordinates")
    {
        ...
        WHEN("We get them coordinates")
        {
            ...
            auto y = point.getY();
            THEN(
                "We realize they both > 0 because chessboard have no negative "
                "coordinates")
            {
                REQUIRE(x > 0);
                REQUIRE(y > 0);
