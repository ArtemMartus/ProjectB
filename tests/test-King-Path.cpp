...
TEST_CASE_METHOD(PPathSystem, "Test the king movement system") {
    WHEN("Castling test") {
        ...
        THEN("Can castle left and right") {
            ...
            for (const auto &i : moves) {
                if (*i.first == *king &&
                        *(i.first->getPoint()) == *(king->getPoint())) {
                    if (*i.second == kingLeftRookPoint) kingCastleLeft = true;
                    if (*i.second == kingRightRookPoint) kingCastleRight = true;
                } else if (*i.first == *leftRook &&
                           *(i.first->getPoint()) == *(leftRook->getPoint())) {
                    if (*i.second == *king->getPoint()) leftRookCastle = true;
                } else if (*i.first == *rightRook &&
                           *(i.first->getPoint()) == *(rightRook->getPoint())) {
        ...
        AND_WHEN(
                    ...
                    "castling") {
            auto rook =
                    make_shared<PFigure>(PPoint(king->getX() - 2, 5), Rook, Blacks);
    ...
    WHEN("There is one king in the middle of a map") {
        auto king = make_shared<PFigure>(PPoint(3, 3), FigureType::King,
                                         FigurePlayer::Whites);
        setBoard({king});
        ...
        THEN("Can move everywhere") {
            REQUIRE(path.size() ==
                    8);  // top topRight right rightBot bot botLeft left leftTop
        }
    }
    WHEN("King moved to the top right corner") {
        auto king = make_shared<PFigure>(PPoint(7, 7), FigureType::King,
                                         FigurePlayer::Whites);
        setBoard({king});
        auto path = buildPath(king);
    ...
    WHEN("King stands on a line with enemy rook") {
        auto king = make_shared<PFigure>(PPoint(7, 7), FigureType::King,
                                         FigurePlayer::Whites);
        auto enemyKing = make_shared<PFigure>(PPoint(0, 0), FigureType::King,
                                              FigurePlayer::Blacks);
        auto enemyRook = make_shared<PFigure>(PPoint(0, 7), FigureType::Rook,
                                              FigurePlayer::Blacks);
        ...
        THEN("King can have only two options of where to move") {
            ...
            for (const auto &i : moves)
                if (*i.first == *king) kingMoves.push_back(i.second);
    ...
    WHEN(
        ...
        auto enemyRook = make_shared<PFigure>(PPoint(0, 7), FigureType::Rook,
                                              FigurePlayer::Blacks);
        auto allyBishop = make_shared<PFigure>(PPoint(7, 0), FigureType::Bishop,
                                               FigurePlayer::Whites);
    ...
    WHEN("King stands near enemy queen") {
        ...
        auto enemyKing = make_shared<PFigure>(PPoint(0, 0), FigureType::King,
                                              FigurePlayer::Whites);
        auto enemyQueen = make_shared<PFigure>(PPoint(6, 6), FigureType::Queen,
                                               FigurePlayer::Whites);
        ...
        AND_WHEN("Somebody is protecting the queen - it's checkmate indeed") {
            auto pawn = make_shared<PFigure>(PPoint(5, 5), FigureType::Pawn,
                                             FigurePlayer::Whites);
