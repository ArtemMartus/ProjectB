...
TEST_CASE_METHOD(PathSystem, "Test the king movement system")
{
    WHEN("Castling test")
    {
        auto king = FigureFactory::buildKing(FigurePlayer::Whites);
        auto enemyKing = FigureFactory::buildKing(FigurePlayer::Blacks);
        auto allyRooks = FigureFactory::buildRooks(FigurePlayer::Whites);
        auto leftRook = allyRooks.front();
        auto rightRook = allyRooks.back();

        auto kingLeftRookPoint = Point(king->getX() - 2, king->getY());
        auto kingRightRookPoint = Point(king->getX() + 2, king->getY());

        setBoard({ king, enemyKing, leftRook, rightRook });

        THEN("Can castle left and right")
        {
            REQUIRE(checkCastling(
                leftRook,
            ...
            bool kingCastleLeft = false, kingCastleRight = false,
                 leftRookCastle = false, rightRookCastle = false;
    ...
    {
        auto king = make_shared<Figure>(Point(3, 3), FigureType::King,
            FigurePlayer::Whites);
        setBoard({ king });
    ...
    {
        auto king = make_shared<Figure>(Point(7, 7), FigureType::King,
            FigurePlayer::Whites);
        setBoard({ king });
        auto path = buildPath(king);
    ...
    WHEN("King stands on a line with enemy rook")
    {
        auto king = make_shared<Figure>(Point(7, 7), FigureType::King,
            FigurePlayer::Whites);
        auto enemyKing = make_shared<Figure>(Point(0, 0), FigureType::King,
            FigurePlayer::Blacks);
        auto enemyRook = make_shared<Figure>(Point(0, 7), FigureType::Rook,
            FigurePlayer::Blacks);
        ...
        THEN("King can have only two options of where to move")
        {
            ...
            list<shared_ptr<Point>> kingMoves;
            for (const auto& i : moves)
                if (*i.first == *king)
    ...
    WHEN(
        "King stands on a line with enemy rook and ally bishop can kill enemy "
        "rook")
    {
        auto king = make_shared<Figure>(Point(7, 7), FigureType::King,
            FigurePlayer::Whites);
        ...
        auto enemyRook = make_shared<Figure>(Point(0, 7), FigureType::Rook,
            FigurePlayer::Blacks);
        auto allyBishop = make_shared<Figure>(Point(7, 0), FigureType::Bishop,
            FigurePlayer::Whites);
        ...
        THEN("Can move with a king or use bishop to kill enemy")
        {
            ...
            list<shared_ptr<Point>> kingMoves;
            list<shared_ptr<Point>> bishopMoves;
            for (const auto& i : moves)
                if (*i.first == *king)
                    kingMoves.push_back(i.second);
    ...
    WHEN("King stands near enemy queen")
    {
        ...
        auto enemyKing = make_shared<Figure>(Point(0, 0), FigureType::King,
            FigurePlayer::Whites);
        auto enemyQueen = make_shared<Figure>(Point(6, 6), FigureType::Queen,
            FigurePlayer::Whites);
        ...
        {
            auto pawn = make_shared<Figure>(Point(5, 5), FigureType::Pawn,
                FigurePlayer::Whites);
