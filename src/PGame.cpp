...
bool PGame::run() {
    ...
    while (!checkboard->onePlayerLeft()) {
        ...
        case 0: {
            ...
            for (const auto &i : availableMoves) freeFigures.insert(i.first);
            ...
            for (const auto &i : availableMoves)
                if (*i.first == *figure &&
                        *(i.first->getPoint()) == *(figure->getPoint()))
...
PGame::~PGame() { checkboard = nullptr; }

std::shared_ptr<PFigure> PGame::selectFigure(
        const set<shared_ptr<PFigure>> &allowed) {
    ...
    auto good = [&](const shared_ptr<PFigure> &f) -> bool {
        for (const auto &i : allowed)
            if (*i == *f)
                if (*i->getPoint() == *f->getPoint()) return true;
        return false;
    };
