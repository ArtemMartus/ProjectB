...
PPathSystem::PPathSystem(std::list<std::shared_ptr<PFigure>> b) noexcept
    : board(std::move(b)) {}
...
list<shared_ptr<PPoint>> PPathSystem::buildPath(
        const shared_ptr<PFigure> &figure) const {
    ...
    if (figure->isPawn()) rawMoves.splice(rawMoves.end(), buildPawnPath(figure));
    ...
    for (const auto &i : rawMoves)
        if (i) possibleMoves.push_back(i);

    return possibleMoves;
}

shared_ptr<PPoint> PPathSystem::addOrDie(const shared_ptr<PPoint> &p,
                                         FigurePlayer side,
                                         bool pawnMode) const {
    if (!p) return nullptr;
    ...
    bool allowNext = p->inBounds() && (pawnMode ? (fig && differentSides)
                                                : (!fig || differentSides));

    return allowNext ? p : nullptr;
}

list<shared_ptr<PPoint>> PPathSystem::buildPawnPath(
        const shared_ptr<PFigure> &figure) const {
    BUILD_PATH_INTRO;
    auto pawnY = side == FigurePlayer::Whites ? 1 : -1;

    auto p = addOrDie(make_shared<PPoint>(x, y + pawnY), side,
                      false);  // cannot attack forward
    if (p && !at(p)) {
        path.push_back(p);
        if (figure->getMovesCount() == 0)
            path.push_back(
                        addOrDie(make_shared<PPoint>(x, y + 2 * pawnY), side, false));
    }
...
list<shared_ptr<PPoint>> PPathSystem::buildKnightPath(
        const shared_ptr<PFigure> &figure) const {
    BUILD_PATH_INTRO;
...
list<shared_ptr<PPoint>> PPathSystem::buildRookPath(
        const shared_ptr<PFigure> &figure) const {
    BUILD_PATH_INTRO;
    ...
    auto king = side == FigurePlayer::Whites ? at(make_shared<PPoint>(4, 0))
...
list<shared_ptr<PPoint>> PPathSystem::buildBishopPath(
        const shared_ptr<PFigure> &figure) const {
    BUILD_PATH_INTRO;
...
list<shared_ptr<PPoint>> PPathSystem::buildKingPath(
        const shared_ptr<PFigure> &figure) const {
    BUILD_PATH_INTRO;
    ...
    auto rook1 = side == FigurePlayer::Whites ? at(make_shared<PPoint>(0, 0))
                                              : at(make_shared<PPoint>(0, 7));
    auto rook2 = side == FigurePlayer::Whites ? at(make_shared<PPoint>(7, 0))
                                              : at(make_shared<PPoint>(7, 7));
...
bool PPathSystem::checkCastling(const shared_ptr<PFigure> &one,
                                const shared_ptr<PFigure> &two) const {
    if (!one || !one->isReadyForCastling() || !two ||
            ...
            (*one == *two &&
             *one->getPoint() == *two->getPoint()))  /// two same figures
...
list<shared_ptr<PPoint>> PPathSystem::checkForAnyMovement(
        const shared_ptr<PFigure> &from) const {
    if (!from) return {};
    return buildPath(from);
}

bool PPathSystem::checkForMovement(const shared_ptr<PFigure> &figure,
                                   const shared_ptr<PPoint> &to) const {
    if (!figure || !to) return false;
    ...
    for (const auto &i : moves)
        if (*i.first == *figure)
            if (*i.second == *to) return true;

    return false;
}

multimap<shared_ptr<PFigure>, shared_ptr<PPoint>>
PPathSystem::getRawListOfMoves(FigurePlayer side) const {
    // ally & enemy figures
    ...
    // trace back method
    auto traceBack = [&](
            const shared_ptr<PFigure> &figure) -> list<shared_ptr<PPoint>> {
        ...
        shared_ptr<PPoint> point = make_shared<PPoint>(ourKing->getPoint()->getX(),
    ...
    for (const auto &i : allies) {
        ...
        for (const auto &pos : path) {
            ...
            if (isInWanted) allyForces.insert({i, pos});
        }
    }
    return allyForces;
}
...
multimap<shared_ptr<PFigure>, shared_ptr<PPoint>>
PPathSystem::getListOfAvailableMoves(FigurePlayer side) const {
    ...
    for (const auto &i : allyForces) {
        ...
        if (possibleFigure) possibleFigure->isCapturedBy(figure);
        ...
        if (!futureBlock) filteredAllies.insert({figure, spot});

        if (possibleFigure) possibleFigure->revive();
    ...
    for (const auto &i : filteredAllies) {
        ...
        const bool isKingNow =
                *i.first == *king && *(i.first->getPoint()) == *(king->getPoint());
        const bool toInsert =
                isKingNow || (figure->isReadyForCastling()
                              ? checkCastling(figure, king) && kingCanCastle
                              : true);
        if (i.second && toInsert) doubleFilteredAllies.insert(i);
    }
